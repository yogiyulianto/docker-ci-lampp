...
class Dokter extends PrivateBase {

    const SESSION_SEARCH = 'search_dokter';
    const PAGE_TITLE = 'Dokter';
    const PAGE_HEADER = 'Dokter';
    const PAGE_URL = 'administrator/account/dokter/';
    ...
    public function __construct() {
        ...
        $this->load->model('administrator/account/M_dokter');
    }
    ...
    public function index() {
        ...
        $total_row = $this->M_dokter->count_all();
        ...
        $result = $this->M_dokter->get_all($rs_search, $config['per_page'], $from);
    ...
    public function add() {
        ...
        $all_role = $this->M_dokter->get_all_role();
        $data = array(
            'rs_roles' => $all_role,
        );
    ...
    public function add_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $user_id = $this->M_dokter->generate_id($prefix, $params_prefix);
            $params = array(
                'user_id' => $user_id,
            ...
            // insert
            if ($this->M_dokter->insert('com_user', $params)) {
                // insert to users
                $params = array(
                    'user_id' => $user_id,
                ...
                $this->M_dokter->insert('user', $params);
                // insert hak akses
                $params = array(
                    'user_id' => $user_id,
                    'role_id' => '2005',
                );
                $this->M_dokter->insert('com_role_user', $params);
                //sukses notif
    ...
    public function edit($user_id = '') {
        ...
        //parsing
        $data = array(
            'result' => $this->M_dokter->get_by_id($user_id),
            'rs_roles' => $all_role,
        );
    ...
    public function edit_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            // insert
            if ($this->M_dokter->update('com_user', $params, $where)) {
                // insert to users
                $params = array(
                    'full_name' => $this->input->post('full_name'),
                    'address' => $this->input->post('address'),
                ...
                $this->M_dokter->update('user', $params, $where);
                //sukses notif\
    ...
    public function delete_process() {
        ...
        //process
        if ($this->M_dokter->delete('com_user', $where)) {
            //sukses notif
