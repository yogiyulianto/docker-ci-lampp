...
class Pasien extends PrivateBase {

    const SESSION_SEARCH = 'search_pasien';
    const PAGE_TITLE = 'pasien';
    const PAGE_HEADER = 'pasien';
    const PAGE_URL = 'administrator/master/pasien/';
    ...
    public function index() {
        ...
        $total_row = $this->M_pasien->count_all();
        ...
        $result = $this->M_pasien->get_all($rs_search, $config['per_page'], $from);
        $data = array(
            'rs_search' => $rs_search,
            'rs_id' => $result,
    ...
    public function add_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $user_id = $this->M_pasien->generate_id($prefix, $params_prefix);
            $params = array(
                'user_id' => $user_id,
            ...
            // insert
            if ($this->M_pasien->insert('com_user', $params)) {
                // insert to pasien
                $params = array(
                    'user_id' => $user_id,
                ...
                $this->M_pasien->insert('user', $params);
                // insert hak akses
                $params = array(
                    'user_id' => $user_id,
                    'role_id' => '02003',
                );
                $this->M_pasien->insert('com_role_user', $params);
                $params = array(
                    'pasien_id' => $user_id,
                    'user_id' => $user_id,
                ...
                $this->M_pasien->insert('pasien', $params);
                //sukses notif
    ...
    public function edit($user_id = '') {
        ...
        //parsing
        $data = array(
            'result' => $this->M_pasien->get_by_id($user_id),
            'rs_roles' => $all_role,
        );
    ...
    public function edit_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            // insert
            if ($this->M_pasien->update('com_user', $params, $where)) {
                // insert to pasien
                $params = array(
                    'full_name' => $this->input->post('full_name'),
                    'address' => $this->input->post('address'),
                ...
                $this->M_pasien->update('user', $params, $where);
                // insert hak akses
                $params = array(
                    'role_id' => '02003',
                );
                $this->M_pasien->update('com_role_user', $params, $where);
                //sukses notif\
    ...
    public function delete_process() {
        ...
        //process
        if ($this->M_pasien->delete('com_user', $where)) {
            //sukses notif
