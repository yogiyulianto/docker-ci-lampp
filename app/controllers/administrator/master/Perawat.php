...
class Perawat extends PrivateBase {

    const SESSION_SEARCH = 'search_perawat';
    const PAGE_TITLE = 'perawat';
    const PAGE_HEADER = 'perawat';
    const PAGE_URL = 'administrator/master/perawat/';
    ...
    public function index() {
        ...
        $total_row = $this->M_perawat->count_all();
        ...
        $result = $this->M_perawat->get_all($rs_search, $config['per_page'], $from);
        $data = array(
            'rs_search' => $rs_search,
            'rs_id' => $result,
    ...
    public function add() {
        ...
        $all_role = $this->M_perawat->get_all_role();
        $data = array(
            'rs_roles' => $all_role,
        );
    ...
    public function add_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            $user_id = $this->M_perawat->generate_id($prefix, $params_prefix);
            $params = array(
                'user_id' => $user_id,
            ...
            // insert
            if ($this->M_perawat->insert('com_user', $params)) {
                // insert to perawat
                $params = array(
                    'user_id' => $user_id,
                ...
                $this->M_perawat->insert('user', $params);
                // insert hak akses
                $params = array(
                    'user_id' => $user_id,
                    'role_id' => '02002',
                );
                $this->M_perawat->insert('com_role_user', $params);
                $params = array(
                    'perawat_id' => $user_id,
                    'user_id' => $user_id,
                ...
                $this->M_perawat->insert('perawat', $params);
                //sukses notif
    ...
    public function edit($user_id = '') {
        ...
        //parsing
        $data = array(
            'result' => $this->M_perawat->get_by_id($user_id),
            'rs_roles' => $all_role,
        );
    ...
    public function edit_process() {
        ...
        if ($this->form_validation->run() !== false) {
            ...
            // insert
            if ($this->M_perawat->update('com_user', $params, $where)) {
                // insert to perawat
                $params = array(
                    'full_name' => $this->input->post('full_name'),
                    'address' => $this->input->post('address'),
                ...
                $this->M_perawat->update('user', $params, $where);
                // insert hak akses
                $params = array(
                    'role_id' => '02002',
                );
                $this->M_perawat->update('com_role_user', $params, $where);
                //sukses notif\
    ...
    public function delete_process() {
        ...
        //process
        if ($this->M_perawat->delete('com_user', $where)) {
            //sukses notif
