...
class Webinar_category extends PrivateBase {

    const PAGE_TITLE = 'Webinar Category';
    const PAGE_HEADER = 'Webinar Category';
    const PAGE_URL = 'administrator/master/webinar_category/';
    ...
    public function __construct() {
        ...
        // LOAD MODEL
        $this->load->model('administrator/master/M_webinar_category');
        // LOAD LIBRARY
    ...
    public function index() {
        ...
        $total_row = $this->M_webinar_category->count_all();
        ...
        //get data 
        $rs_id = $this->M_webinar_category->get_all($from ,$config['per_page']);
        $data = array(
            'rs_id' => $rs_id,
    ...
    public function add() {
        ...
        // get data & parse
        $data = array(
            'rs_category' => $this->M_webinar_category->get_all_category(), 
        );
        // render view
    ...
    public function add_process() {
        ...
        if ($this->form_validation->run() !== FALSE) {
            if (!empty($_FILES['image']['tmp_name'])) {
                // upload config
                $config['upload_path'] = 'assets/images/category_webinar/';
                ...
                if ($this->tupload->do_upload_image('image', 128, false)) {
                    ...
                    $file_name = 'assets/images/category_webinar/'.$data_upload['file_name'];
                }
            ...
            // insert
            if ($this->M_webinar_category->insert('category_webinar', $params)) {
                //sukses notif
    ...
    public function edit($category_id = '') {
        ...
        $data = array(
            'rs_category' => $this->M_webinar_category->get_all_category(), 
            'result' => $this->M_webinar_category->get_by_id($category_id),
    ...
    public function edit_process() {
        ...
        if ($this->form_validation->run() !== FALSE) {
            ...
            // insert
            if ($this->M_webinar_category->update('category_webinar', $params, $where)) {
                //sukses notif
    ...
    public function delete_process() {
        ...
        //process
        if ($this->M_webinar_category->delete('category_webinar', $where)) {
            //sukses notif
