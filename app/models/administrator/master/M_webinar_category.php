...
class M_webinar_category extends MY_Model {
    ...
    public function get_all($from ,$page) {
        $sql = "SELECT * FROM category_webinar a LIMIT $from ,$page";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
    ...
    public function get_all_blog($base_url) {
        ...
        b.title as 'category_title' FROM blogs a JOIN category_webinar b ON a.category_id = b.category_id 
        WHERE blog_st != 'draft'";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $result = $query->result_array();
    ...
    public function get_by_id($category_id) {
        ...
        $this->db->where('category_webinar.category_id', $category_id);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $result = $query->row_array();
            $query->free_result();
