				...
				<div class="row">
					<div class="col-12">
						...
						<div class="form-group row">
							...
							<div class="col-lg-6">
								...
								<select name="audio_st" class="select-2 " style="width:100%">
									...
									<option value="published" {{ set_select(old_input('audio_st') , 'published') }}> Published </option>
									<option value="draft" {{ set_select(old_input('audio_st') , 'draft') }}> Draft </option>
								</select>
								<div class="error text-danger">{{error_form('category_id') ?? ''}}</div>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-md-6">
								<label>Audio*</label><small> Hanya Audio (mp3) maks. 10MB </small>
								<input type="file" class="dropify {{error_form_class('audio')}}" name="audio"
									data-show-errors="true" data-allowed-file-extensions="mp3" data-max-file-size="10M">
